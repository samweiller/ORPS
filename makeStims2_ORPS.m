...
function makeStims2_ORPS(numImages)
...
for set = 1:numStimSets
    ...
    chosenImages = textscan(fileIO, '%s', 'Delimiter', {'\n'});
        chosenImagesIndex = randsample(size(chosenImages{1},1), numImages);
        for i = 1:size(chosenImagesIndex, 1)
            fprintf('Reading %s...\n', chosenImages{1}{chosenImagesIndex(i)});
            STIMNAMES{set}{i} = chosenImages{1}{chosenImagesIndex(i)};
            STIMS{set}{i} = imread(chosenImages{1}{chosenImagesIndex(i)});
        end;
...
save ORPSstims.mat STIMS stimDirs STIMNAMES
